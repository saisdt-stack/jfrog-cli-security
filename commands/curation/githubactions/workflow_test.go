...
func TestCollectSubpaths_DeduplicatesAndPreservesFirstSeenOrder(t *testing.T) {
	// The same subpath used twice contributes one entry, and the order is the file's - the
	// report's Action cell renders this slice verbatim.
	got := collectSubpaths([]WorkflowUse{
		{Owner: "github", Repo: "codeql-action", Ref: "v3", Subpath: "init"},
		{Owner: "github", Repo: "codeql-action", Ref: "v3", Subpath: "analyze"},
		{Owner: "github", Repo: "codeql-action", Ref: "v3", Subpath: "init"},
		{Owner: "actions", Repo: "checkout", Ref: "v4", Subpath: ""},
	})

	assert.Equal(t, []string{"init", "analyze"}, got[refKey("github", "codeql-action", "v3")])
	assert.NotContains(t, got, refKey("actions", "checkout", "v4"), "an empty subpath contributes nothing")
}
