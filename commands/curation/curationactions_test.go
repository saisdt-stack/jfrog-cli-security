...
type runnerSpec struct {
	...
	fixtureCache bool
	// cacheDirs are "owner/repo/ref" entries to create on top of that.
	cacheDirs []string
...
func (s runnerSpec) build(t *testing.T) (workingDir, actionsCacheDir string) {
	...
	for _, dir := range s.cacheDirs {
		require.NoError(t, os.MkdirAll(filepath.Join(actionsCacheDir, filepath.FromSlash(dir)), 0755))
	}
...
func TestCurationActionsCommand_Run_ExitStatus(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			name: "verify when the child of a local composite action is rejected then the command fails",
			spec: runnerSpec{
...
func TestCurationActionsCommand_Run_WorkflowFileResolution(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			name:            "verify when the derived workflow file is malformed then the command fails",
			spec:            runnerSpec{cacheDirs: oneAction, workflowYAML: "jobs:\n\t- this is not valid yaml\n"},
			workflowRef:     derivedWorkflowRef,
			wantErrContains: "parsing workflow file",
		},
		{
